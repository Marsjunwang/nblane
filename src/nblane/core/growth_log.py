...
def append_growth_log_row(profile_dir: Path, event: str) -> None:
    ...
    The whole read → modify → write cycle holds the SKILL.md sidecar
    lock so a concurrent SKILL.md writer (sync, another growth event)
    ...
    with locked_profile_write(profile_dir, "SKILL.md"):
