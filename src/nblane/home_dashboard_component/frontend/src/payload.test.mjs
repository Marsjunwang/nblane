...
import { goalDisplay, goalDraftFromFormData, hiddenArchivedSubtreeIds, normalizePayload } from "./payload.js";
import {
  archiveGoalEvent,
...
// Regression: hiding an archived/paused goal must take its WHOLE work subtree
// with it. Otherwise the orphaned projects/tasks/evidence get adopted into the
// synthetic "Other" hub (3D) or flung onto a stray ring (2D), so toggling the
// goal off paradoxically relocated its work into view instead of removing it.
// Roles mirror the real payload: goal=direction, project=branch, task/output=
// leaf, evidence=fruit, skill=star. Relations mirror the real graph too:
// goal--contains-->project, project--contains-->task, task--generated_by-->
// evidence, and the shared-hub edges (task--supports-->skill, etc.) that used
// to make the cascade run away across the whole graph.
const archivePayload = () => ({
  graph: {
    nodes: [
      { id: "goal:live", type: "goal", role: "direction", status: "active" },
      { id: "goal:gone", type: "goal", role: "direction", status: "archived" },
      { id: "goal:paused", type: "goal", role: "direction", status: "paused" },
      { id: "proj:a", type: "project_case", role: "branch", status: "" }, // under archived goal only
      { id: "task:a", type: "kanban_task", role: "leaf", status: "" }, // under proj:a
      { id: "ev:a", type: "atomic_evidence", role: "fruit", status: "" }, // generated by task:a
      { id: "proj:shared", type: "project_case", role: "branch", status: "" }, // under BOTH live + paused
      { id: "skill:x", type: "skill", role: "star", status: "solid" }, // shared hub
      { id: "north_star", type: "north_star", role: "trunk", status: "" },
    ],
    edges: [
      { from: "north_star", to: "goal:live", relation: "alignment" },
      { from: "north_star", to: "goal:gone", relation: "alignment" },
      { from: "north_star", to: "goal:paused", relation: "alignment" },
      { from: "goal:gone", to: "proj:a", relation: "contains" },
      { from: "proj:a", to: "task:a", relation: "contains" },
      { from: "task:a", to: "ev:a", relation: "generated_by" },
      { from: "goal:paused", to: "proj:shared", relation: "contains" },
      { from: "goal:live", to: "proj:shared", relation: "contains" },
      // Shared-hub edges that must NOT drag the whole graph into the cascade:
      { from: "task:a", to: "skill:x", relation: "supports" },
      { from: "proj:shared", to: "skill:x", relation: "supports" },
    ],
  },
});

test("hiddenArchivedSubtreeIds hides extinguished goals and their work subtrees", () => {
  const hidden = hiddenArchivedSubtreeIds(archivePayload(), false);
  // Both extinguished goals + the archived goal's exclusive subtree (down to
  // evidence via generated_by) are hidden.
  assert.ok(hidden.has("goal:gone"));
  assert.ok(hidden.has("goal:paused"));
  assert.ok(hidden.has("proj:a"), "project under archived goal must be hidden");
  assert.ok(hidden.has("task:a"), "task under archived goal must be hidden");
  assert.ok(hidden.has("ev:a"), "evidence under archived goal must be hidden");
  // The active goal stays, work it shares with a paused goal stays, and the
  // shared skill hub is never collateral (would make the cascade run away).
  assert.ok(!hidden.has("goal:live"));
  assert.ok(!hidden.has("proj:shared"), "work shared with a live goal must survive");
  assert.ok(!hidden.has("skill:x"), "shared skill hub must never be hidden");
  assert.ok(!hidden.has("north_star"));
});

test("hiddenArchivedSubtreeIds returns empty when toggle on or nothing archived", () => {
  assert.equal(hiddenArchivedSubtreeIds(archivePayload(), true).size, 0);
  const allActive = { graph: { nodes: [{ id: "g", type: "goal", role: "direction", status: "active" }], edges: [] } };
  assert.equal(hiddenArchivedSubtreeIds(allActive, false).size, 0);
  assert.equal(hiddenArchivedSubtreeIds({}, false).size, 0);
});
