...
def _ui() -> dict[str, str]:
    if llm_client.ui_language() == "zh":
        return {
            ...
            "ai_action_reorganize": "整理全文",
            "ai_action_reorganize_hint": "在不删除内容的前提下整理全文格式",
...
def _blog_meta_from_state(
    ...
    *,
    ...
    return {
        ...
        "related_sources": _csv_values(
            st.session_state.get(
                _blog_editor_key(selected, slug, "related_sources"),
                _csv_text(original_meta.get("related_sources")),
            )
        ),
        "related_research_claims": _csv_values(
            st.session_state.get(
                _blog_editor_key(selected, slug, "related_research_claims"),
                _csv_text(original_meta.get("related_research_claims")),
            )
        ),
        "related_citations": _csv_values(
            st.session_state.get(
                _blog_editor_key(selected, slug, "related_citations"),
                _csv_text(original_meta.get("related_citations")),
            )
        ),
    }
...
def _render_blog_meta_panel(
    *,
    ...
    st.text_input(
        ui["related_sources"],
        value=_csv_text(meta.get("related_sources")),
        key=_blog_editor_key(selected, slug, "related_sources"),
    )
    st.text_input(
        ui["related_research_claims"],
        value=_csv_text(meta.get("related_research_claims")),
        key=_blog_editor_key(selected, slug, "related_research_claims"),
    )
    st.text_input(
        ui["related_citations"],
        value=_csv_text(meta.get("related_citations")),
        key=_blog_editor_key(selected, slug, "related_citations"),
    )
