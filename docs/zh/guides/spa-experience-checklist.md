...
| 1.4 | admin 登录后，左侧导航 16 项逐一点一遍 | 每页都能打开；当前页高亮正确（「证据评审」已并入「证据」单页，旧路由 /evidence-review 重定向到 /evidence?stage=review） | ☐ |
...
| # | 步骤 | 预期 | 结果 |
|---|------|------|------|
| 1 | 左栏五阶段（待结晶/待评审/已入座/待补强/已废弃） | 数字常驻且与各阶段列表一致 | ☐ |
| 2 | 待评审：选中条目 → 铭文详情卡 → 接受/拒绝 | 成功后条目离开队列；拒绝进「已废弃」，可恢复 | ☐ |
| 3 | 快速评审模式 | j/k 移动、a 接受、s 跳过、1/2/3 定强度（弱/中/强） | ☐ |
| 4 | 详情卡评级下拉（强度/置信度/公开就绪）+ 编辑（标题/摘要/日期/链接） | 值写回 evidence-pool.yaml | ☐ |
| 5 | 技能关联：建议 chip 一键关联 / 点已关联 chip 取消 | 写入 skill-tree.yaml evidence_refs；死链看板引用显示「已归档」墓碑不报错 | ☐ |
| 6 | 「从已完成任务结晶」三步向导 | 选 Done → 规则/AI 草稿 → 评级勾选入库；任务回标 crystallized | ☐ |
| 7 | 旧路由 /evidence-review | 重定向到 /evidence?stage=review，不 404 | ☐ |
...
| 4.2 | 两标签开证据单页，A 接受条目后 B 对同一条目改评级 | B 412 提示 | ☐ |
