...
- 2026-09-24 习惯生命周期 + 技能进阶后端(habit archive/delete 端点、证据 `breakthrough` 字段、`GET /skill-tree` 节点 progress、`skill.lit`/`habit.deleted` chronicle kind):**无新增依赖**——纯 Python(`core/activity_log.py` 归档/删除助手、`core/skill_progression.py` 调参常量、projects-board `include_archived` 口径扩展)复用既有写锁/快照/ETag 纪律与 FastAPI 栈;无新数据资产、无新端口/服务/包,profile 侧无新文件(只扩 activity-log.yaml/evidence-pool.yaml 既有字段)。
- 2026-09-25 项目页 someday 进入日常流程(徽章卡「列入 Queue」「标记 Done」):**无新增依赖**——复用既有 kanban move/done 端点与 Mantine/react-query 存量;无新端口/服务/包,profile 侧无新文件。
- 2026-09-24 看板任务删除(`DELETE /kanban/cards/{card_ref}`、`task.deleted` chronicle kind、TaskDetailCard 危险操作):**无新增依赖**——纯 Python(routes_v1/schemas 复用 kanban_io find_kanban_card + save_kanban_with_merge 锁/快照/3-way merge 纪律、`core/chronicle.py` 追加一个 kind),前端复用 Mantine/react-query 存量;无新数据资产、无新端口/服务/包,profile 侧无新文件。
- 2026-09-24 openclaw 留痕与 git actor(G1/G2,production-launch-plan §3.2):**无新增依赖**——纯 Python(routes_v1 `_record_agent_writeback` 复用 `core/review_actions.record_writeback_activity`,web_api `GitActorMiddleware` 纯 ASGI 中间件复用 `core/git_backup.start_operation` 与既有 cookie 会话解析);无新端口/服务/包,profile 侧无新文件(只向既有 agent-activity.yaml 追加 writeback 条目)。
- 2026-09-24 生产上线 C 尾清理包(占卜「化为任务」桥、健康页解散、突破徽章投影、P2 四条):**新增前端 npm 依赖 `@mantine/dates@^8.3.18` + `dayjs@^1.11.23`**(排期 DateInput;原生 type=date 的显示格式跟随浏览器 UI locale 不可控,valueFormat 路线才可靠;样式经 main.tsx 引入,产物随 web_ui/static 提交)——Python/后端零新增,无新端口/服务。
- 2026-09-24 openclaw nblane_api 客户端(production-launch-plan §3.1,`scripts/openclaw/skills/bin/nblane_api.py`):**无新增依赖**——httpx 已是主依赖(pyproject.toml);无新端口(走既有 8504 loopback),无新服务;会话 cookie 存 `~/.cache/nblane/api-cookies.json`(0600,不入库);密码只读环境变量 `NBLANE_OPENCLAW_API_PASSWORD`(已登记 .env.example/§3.1)。
