...
**配置 Codex** 大弹窗，可编辑 Codex CLI 的 `~/.codex/config.toml`，也可打开并
编辑当前 profile 的 `codex.yaml`；profile 配置文件不保存 token 或 API key。
API key/auth 由 Codex CLI 管理，Web 只通过 `codex login --with-api-key` 写入。
