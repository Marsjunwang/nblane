...
本文是公开站 Blog 编辑器的产品 / 技术设计稿。Public Site 页面已落地初版；
Blog 区现在已经接入 React / BlockNote Editor Shell，包含文章栏、右侧
Meta / Media / AI / Check 工具抽屉、专注模式、Markdown 源码安全模式和
browser `localStorage` 布局记忆。本文继续作为后续打磨边界：在保持
`profiles/<name>/blog/*.md` 为内容事实源的前提下，把当前 shell 的
新建/上传、块级插入和 AI 候选体验继续做深。

## 1. 背景与目标

当前 `Public Site -> Blog` 已经能创建、编辑、预览、插入媒体、生成 AI 候选和
发布博客；React shell 已经承接主体写作界面，Streamlit 继续承接文件 I/O、
上传、AI 调用与发布校验。距离更完整的块编辑体验还有差距：

- shell 内新建文章、上传媒体仍由 Streamlit 辅助工具兜底，后续应收进同一
  React 事件流。
- front matter、媒体、AI 候选和发布检查已进入 shell，但候选生成来源仍偏简化。
...
- 更可靠的 browser `localStorage` 布局恢复。
- 光标位置与选中块感知。
- 块级插入、替换、拖拽与 slash menu。
- 窄屏 overlay drawer 或 `Editor / Articles / Tools / Preview` 顶层 tabs。
