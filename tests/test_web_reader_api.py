...
class TestWebReaderApi(unittest.TestCase):
    ...
    def test_view_with_cookie_renders_standalone_reader_html(self) -> None:
        ...
        self.assertIn("const PANEL_MIN_WIDTH = 220;", response.text)
        self.assertIn("const PANEL_MAX_WIDTH = 920;", response.text)
        self.assertIn("clampPanelWidth", response.text)
        self.assertIn("panelWidthBounds", response.text)
        self.assertIn("--pr-panel-min-width: 220px;", response.text)
        self.assertIn("--pr-resize-rail-width: 8px;", response.text)
        self.assertIn(".pr-compare-divider,", response.text)
        self.assertIn(".pr-panel-resize {", response.text)
        self.assertIn(".pr-workspace.panel-collapsed .pr-panel-resize { display: none; }", response.text)
        self.assertIn('class="pr-panel-resize" id="panelResize"', response.text)
        self.assertIn('panelHandle.classList.add("resizing")', response.text)
